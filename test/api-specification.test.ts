...
it(test.name, async () => {
  const spec = testcase.getSpec(
    `testcases/${test.name}/specification.yml`
  ) as any;
  ...
  const expected = testcase.decodeExpectedIssuance(
    `testcases/${test.name}/sd_jwt_issuance.txt`
  );
  ...
  const holder = new SD.Holder({
    alg: "ES256",
    digester: SD.digester("sha-256"),
    signer: spec.key_binding
      ? await SD.JWS.signer(settings.key_settings.holder_key)
      : undefined,
  });
  ...
  const expected2 = testcase.decodeExpectedIssuance(
    `testcases/${test.name}/sd_jwt_presentation.txt`
  );
  ...
  const expectedVerifiedClaim = JSON.parse(
    JSON.stringify(spec.get("expect_verified_user_claims"))
  );
...
describe("testcases", () => {
  for (const test of testcases) {
    ...
    it(test.name, async () => {
      ...
      expect(computed2.decoded).toEqual(expected2.decoded);
      expect(computed2.parsed.disclosures).toEqual(
        expected2.parsed.disclosures
      );
      const issuerVerifier = {
        verify: async (token: string) => {
          const parsed = SD.Parse.compact(token);
          const verifier = await SD.JWS.verifier(
